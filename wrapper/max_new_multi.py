...
def find_single_match_ALL(corpus, args, f, classifier):
    print_corpus(corpus, '\n')
    print '###Generating Windowsize corpus###'
    ...
    for index in range(0, corpus_size):
        ...
        for x in range(0, corpus_size):
            #fuck, en->zh
            real_corpus.append([corpus[x][0], corpus[index][1]])
    ...
    for single_dog in saver_wish_list:
        ...
        if (len(saver_wish_list[single_dog]) > 1):
            boss = max(saver_wish_list[single_dog],key=lambda item:item[1])
            #saver_wish_list[single_dog] = boss
            f.write(str(single_dog)+' ||| '+ ' ||| '.join(boss) + '\n')
        else:
    ...
    for index in range(0, f_size):
        if index % corpus_size == 0:
            if wish_list:
                ...
                #f.write('--------------\n')
                #add by 7/17/2017
                f.write(' ||| '.join([str(zh_line),str(en_line),"{0:.3f}%".format(wish_list[h_index][0] * 100), "{0:.3f}%".format((1-wish_list[h_index][0]) * 100), ' '.join(map(str, ori_corpus[h_index][1])),  ' '.join(map(str, ori_corpus[h_index][0]))]) + '\n')
                num_match += 1
                wish_list.clear()
            #
            now_index += 1
...
def process_xml_corpus(en_dir, zh_dir, args, full_name, classifier):
    ...
    for x in range(0, l_docs):
        ...
        if (args['m_all']):
            find_single_match_ALL(corpus_ok, args, f, classifier)
        else:
        ...
        #f.write('------------------\n')
        #add by 7/17/2017
        #f.write(' ||| '.join([str(index+1), "{0:.3f}%".format(ok*100), "{0:.3f}%".format(wrong*100), ' '.join(map(str, ori_corpus[index][1])).strip(),  ' '.join(map(str, ori_corpus[index][0])).strip()]) + '\n')
        f.write(str(doc_id)+'\n')
        f.write('------------------\n')
